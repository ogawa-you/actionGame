...
// 2019 10 10PM   オブジェクトレイヤーで垂直反転　水平反転　回転の情報追加、複数タイル追加可能 
...
public:
	...
	Object_(int _id, String _name, unsigned _gid, int _x, int _y, int _width, int _height, double _rotation=0)
	{
		Init(_id, _name, _gid, _x, _y, _width, _height,_rotation);
	}
	void Init(int _id, String _name, unsigned _gid, int _x, int _y, int _width, int _height, int _rotation=0)
	{
		id = _id;
		name = _name;
...
public:
	...
	{
		auto ParseUnsigned = [](String str)
		{
			int size = str.size();
			int p10 = 4;
			return (size < 5) ?
				(unsigned)Parse<int>(str) :
				(unsigned)Parse<int>(str.substr(0, size - p10)) * (unsigned)std::pow(10, p10)
				+ Parse<int>(str.substr(size - p10, p10));
		};
		while (XMLelement.name() == U"object")
		{
			// 高さ分、Y座標を引く(TiledmapEditor のバグ？仕様？)
			int height = Parse<int>(XMLelement.attribute(U"height").value());
			

			object.emplace_back(
				Parse<int>(XMLelement.attribute(U"id").value()),
				XMLelement.attribute(U"name").value_or(U""),
				ParseUnsigned(XMLelement.attribute(U"gid").value()),	// オーバー範囲外エラー
				Parse<double>(XMLelement.attribute(U"x").value()),
				Parse<double>(XMLelement.attribute(U"y").value())-height,
				Parse<int>(XMLelement.attribute(U"width").value()),
				height,
				Parse<double>(XMLelement.attribute(U"rotation").value_or(U"0"))	//存在確認　※
			);
...
//	Texture texture;	// うまく読めない
	...
	void SetElement(XMLElement& tmxElement)
	{
		...
		if (tileset.attribute(U"source").has_value())
		{
			...
			// 後でファイルsourceのファイルをロードしてtilesetのデータを追加
			// bug ややこしいのでここでロードしてしまう　@@
			XMLReader tsxfile(source);
			auto errmsg = L"";
			assert(! tsxfile.isNull()); // 
			//errmsg = Parse<wchar_t>(source);
			_ASSERT_EXPR(! tsxfile.isNull(),L"TSXファイルがありません");
...
public:
	...
	ObjectGroup objectGroup;	// 中でObject_のArray 　　objectGroupも配列化が必要？
	// オブジェクトの配列
	...
	*/
	...
	{
		Clear();
		folder = FileSystem::ParentPath(tmxFile);
		XMLReader tmx(tmxFile);
		FileSystem::ChangeCurrentDirectory(folder);
...
//		String nameTileset= readElement.name(); // check
		...
		//<objectGroup>	//※配列化していない
		while (readElement.name() == U"objectgroup")
		{
			objectGroup.id = Parse<int>(readElement.attribute(U"id").value());
		...
		auto testStr = testElm.nextSibling().name();// debug 無いとU"" 
		...
		//TextureAsset::Register(TextureFilename(), TextureFilename());	// 無くす予定　※
//		tileset[0].SetTexture();
//		texture = TextureAsset(TextureFilename());// 高速化 //※
		//texture = tileset[0].texture;
