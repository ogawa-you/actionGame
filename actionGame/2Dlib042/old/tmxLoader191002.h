	...
	void ObjectArraySet(XMLElement XMLelement) //void ObjectArraySet(XMLElement XMLelement)
	{
		while (XMLelement.name() == U"object")
		{
			...
			object.emplace_back(
				...
				Parse<int>(XMLelement.attribute(U"gid").value()),
...
public:
	...
	int firstgid;
	// TSX ファイルパス
	String source // 別TSXファイルで読み込む場合ファイル名がある。 それ以外ない
		, name;
	...
	void SetTexture()
	{
		...
		texture = TextureAsset(image.source);
		//assert(false);
	}
...
//		texture = TextureAsset(TextureFilename());// 高速化 //※
	...
	void TileDraw(int tileNumber, int x, int y)
	{
		...
		int tileId;
		for (tileId = (int)tileset.size() - 1; tileId > 0; --tileId)
		{
			if (tileNumber > tileset[tileId].firstgid)
				break;
		}
		assert(tileId>=0 && " タイル番号が異常です ");

		//texture
		//(GetTileRect(tileNumber)).draw(x, y);	// ※ 下のは配列外エラーになる

		auto& tile = tileset[tileId];
		tile.image.tileTexture
		(GetTileRect(tile.firstgid,tileNumber)).draw(x, y);	// ※ 下のは配列外エラーになる
