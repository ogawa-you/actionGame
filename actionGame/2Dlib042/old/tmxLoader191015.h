...
public:
	...
	Object_(int _id, String _name, unsigned int _gid, int _x, int _y, int _width, int _height, double _rotation=0)
	{
		Init(_id, _name, _gid, _x, _y, _width, _height,_rotation);
	}
	void Init(int _id, String _name, unsigned int _gid, int _x, int _y, int _width, int _height, double _rotation=0)
	{
		id = _id;
		name = _name;
	...
	void ObjectArraySet(XMLElement XMLelement) //void ObjectArraySet(XMLElement XMLelement)
	{
		while (XMLelement.name() == U"object")
		{
			...
			object.emplace_back(
				...
				Parse<unsigned int>(XMLelement.attribute(U"gid").value()),
