...
public:
	...
	*/
	...
	// sample1 <tile id = "0" type = "種類" probability = "3">
	// sample2 <tile id="2">

	// <tile> elementValue </tile> のロード
	// element は nextSibiling に変化するので注意
	...
	// <tile id = "整数" type = "???" probability = "整数"> のロード
	// element は nextSibiling に変化するので注意
...
private:
	...
	*/
	...
	// TMXファイルの <tileset ～Attribute～> Attributeを読み取り、設定する
	// TSXファイルの場合、自動的に切り替える
	void ReadAttributeValue(XMLElement& tilesetElm)
	{
		...
		if (IsTSXfile) // TSXファイル内のTileset属性
		{
		...
		// TMXファイル内のTileset属性 TSXファイル有無で変わる
		...
		{
			Tileset::source = TSXfilePath.value();//TSXファイルがある
		}
		else
		{
			Tileset::name	= tilesetElm.attribute(U"name").value();
...
*/
	...
	// TSXファイルの <tileset ～Attribute～> Attributeを読み取り、設定する
	// TMXファイルの場合、エラーになるので注意
	...
	{
		// 対策済みのつもり：：：ここで <tileset firstgid="1" source="../TileData/ObjectElement.tsx"/> のような感じだとエラー
		ReadAttributeValue(tmxElement);
		auto& tileset = tmxElement;
		//if (tileset.attribute(U"source").has_value())//TSXファイルがある----
		if (Tileset::IsSource())//TSXファイルがある----
		{
			//Tileset::source = tileset.attribute(U"source").value();//TSXファイルがある
			...
			XMLReader TSX_xml((StringView)TSXpath);	// TSXファイルのロード
			Tileset::ReadAttributeValueTSXfile(TSX_xml);

			TSXFile_Set_TilesetAttr_and_Tile(TSX_xml); // TSXファイルから値をセット
			TSX_xml.close();
		}
...
public:
	...
	//  sample <tileset firstgid="1" source="../TileData/ObjectElement.tsx"/>

	/// <summary>
	/// ＜tileset属性 source＞があればロードして追加設定
	/// </summary>
	void LoadTSXfile()
	{
		>>>>>>
