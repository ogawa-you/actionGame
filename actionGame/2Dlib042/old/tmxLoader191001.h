...
public:
	...
	void ObjectArraySet(XMLElement XMLelement) //void ObjectArraySet(XMLElement XMLelement)
	{
		while (XMLelement.name() == U"object")
		{
			...
			int height = ParseInt<int>(XMLelement.attribute(U"height").value());
			object.emplace_back(
				...
				ParseFloat<double>(XMLelement.attribute(U"x").value()),
				ParseFloat<double>(XMLelement.attribute(U"y").value())-height,
...
public:
	...
	int firstgid;
	String source // 別TSXファイルで読み込む場合ファイル名がある
		, name;
	Texture texture; // ここに入れる予定 ※
