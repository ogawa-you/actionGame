...
public:
	...
	{
		//int tileID = 1 + Parse<int>(tileElm.attribute(U"id").value());// 1ずれているので補正
		//auto probabilityAttr = tileElm.attribute(U"probability");
		//if (probabilityAttr.has_value())
		//	TryParse<int>(probabilityAttr.value(),probability);
		//auto typeAttr = tileElm.attribute(U"type");
	...
	*/
	...
	/// <summary>
	/// ＜tile＞～～～～＜/tile＞ の読み込み 
	/// image objectgroup properties のいずれかがある。
	/// </summary>
	/// <param name="element"></param>
	void ReadValue(XMLElement& element)
	{
		...
		for (XMLElement& elm=element;  ; element.nextSibling())
		{
			if (elm.name() == U"image")
			{
				image.ReadAttribute(elm);
			}
			if (elm.name() == U"objectgroup")
			{
				objectGroup.ObjectArraySet(element);
			}
			if (elm.name() == U"properties")
			{
				XMLElement propertyElm = elm.firstChild();
...
*/
	...
	/// ＜tileset＞のロード
	/// </summary>
	/// <param name="tmxTileset"></param>
	void SetElementValue(XMLElement& tmxTileset)
	{
		ReadAttributeValue(tmxTileset); //<tileset ～～～>の読み込み
		...
		while (tilesetChild.name() == U"tile")
		{
			...
			Tile tileAttr(tilesetChild);
			tile[tileAttr.id]=tileAttr;
			tile[tileAttr.id].ReadValue(tilesetChild);
...
public:
	...
	void LoadTMXfile(String tmxFile)
	{
		...
		//String parentName = parentElement.name();
