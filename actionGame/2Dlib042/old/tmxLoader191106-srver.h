...
public:
	static inline String UseFolder;
	// 
	// TMXFile::UseFolder に fileName にあるフォルダを取得
	//
	// 引数：ファイルネーム, 階層（通常は０のまま）
	// 戻り値無し　※ TMXFile::UseFolder にセットされる
	// 例： U"abcd/efg.tmx" >> TMXFile::UseFolder=U"abcd/"
	static void SetUseFolder(String fileName, int parent = 0)
	{
		UseFolder=fileName.substr(parent, fileName.lastIndexOf(U'/') + 1);
	}
	// 
	// フォルダ付きファイル名取得
	//
	// 引数：ファイルネーム
	// 戻り値：（TMXFile::UseFolder + fileName）
	// 例： U"efg.tmx" >> TMXFile::UseFolder=U"abcd/efg.tmx"（U"abcd/"は既にある前提）
	static String SetName(String fileName)
	{
		return TMXfile::UseFolder + fileName;
	}
};
...
public:
	Object_(XMLElement& xmlElement)
	{
		...
		Init(
			...
			Parse<int>(xmlElement.attribute(U"height").value_or(U"0")),	//error無い場合がある
	...
	Object_(int _id, String _name, int _gid, int _x, int _y, int _width, int _height, double _rotation = 0, String valueName = U"")
	{
		Init(_id, _name, _gid, _x, _y, _width, _height, _rotation, valueName);
	}
private:
	// objectの初期化、注意：valueNameがあっても、さらにそのデータの追加まではここではやらない
	void Init(int _id = 0, String _name = U"", int _gid = 0, int _x = 0, int _y = 0, int _width = 0, int _height = 0, double _rotation = 0, String valueName = U"")
	{
		id = _id;
		name = _name;
		...
		x = _x;
		y = _y - _height;// 高さ分、Y座標を引く(TiledmapEditor のバグ？仕様？)
		width = _width;
		height = _height;
		rotation = _rotation;
...
public:
	...
	void SetElement(XMLElement& tmxElement)
	{
		...
		if (tileset.attribute(U"source").has_value())
		{
			...
			auto TSXpath = TMXfile::SetName(source); // TMXがユーザー作成フォルダ内ならフォルダ階層が深くなり、ファイル名にフォルダしてしなければならなくなるため
...
public:
	...
	*/
	...
	//inline String GetParentPass(String path, int parent = 0)
	//{
	//	return path.substr(parent, path.lastIndexOf(U'/') + 1);
	//}

	void LoadTMXfile(String tmxFile)
	{
		Clear();
		//rootFolder = FileSystem::CurrentDirectory();	//test
		//parentFolder = FileSystem::ParentPath(tmxFile);
		auto rootFolder00 = FileSystem::CurrentDirectory();
		auto parentFolder00 = FileSystem::ParentPath(tmxFile);
		//d rootFolder = U"";
		TMXfile::SetUseFolder(tmxFile);
		/*{
			FileSystem::ChangeCurrentDirectory(U"Texture");	//test
			Texture testg(U"Cursor.png");
			Texture testgxx(U"dumy.png");
			auto g = testg.isEmpty();
			auto gxx = testgxx.isEmpty();
		}*/
		...
		XMLReader tmx((StringView)tmxFile);
		//FileSystem::ChangeCurrentDirectory(parentFolder); //todo
...
//		String nameTileset= readElement.name(); // check
		...
		for (auto& a : tileset)
		{
			...
			auto file = TMXfile::SetName(source);
			_ASSERT_EXPR(FileSystem::IsFile(FilePath(file)), L"タイルの画像が読み込めません。ファイル名ミスの可能性があります。");
			...
			//		return &mesL;
			//	}(source));
			TextureAsset::Register(file, file);
			a.image.tileTexture = TextureAsset(file);
		}
		//FileSystem::ChangeCurrentDirectory(rootFolder);	// フォルダパスを戻す  todo
	...
	{
		return Rect(scrollPosition.x, scrollPosition.y, Scene::Size()).intersects(pos);
	}
	...
	//	return Rect(scrollPosition.x, scrollPosition.y, Scene::Size()).intersects(drawRect);
	//}
