...
// 2021 1/26 TSXファイル中の<tile>内容対応、アニメーションタイル機能対応 テラインは未実装
// 2021 2/3  ロードファイルのフォルダ階層が多岐にわたる場合のバグ修正、ただし <tile>内の<image>は非対応
...
public:
	...
	/// 現在進行形のルートフォルダ
	/// </summary>
	static inline String RootFolder;
	...
	/// <param name="parent">階層の深度（通常は０のまま）</param>
...
// 例： (U"abcd/efg.tsx",U"apple.png",0) >> U"abcd/apple.png"

	/// <summary>
	///  フォルダ付きファイル名取得
	/// </summary>
	/// <param name="fileName">ファイルパス１</param>
	/// <param name="addFile">PNGファイル名</param>
	/// <param name="parent">フォルダ階層（通常は０）</param>
	/// <returns>ファイルパス１の階層の画像ファイル名</returns>
	static String GetPNGFile(String fileName, String addFile, int parent = 0)
	{
		return fileName.substr(parent, fileName.lastIndexOf(U'/') + 1)+addFile;
	}
...
public:
	...
	// 例：<image width="200" height="200" source=".png/SavePoint.png"/> のロード
...
public:
	...
	*/
	...
	/// アニメーションのトータルタイム
	/// </summary>
	/// <returns>アニメーションのトータル時間（ms）</returns>
	int GetAnimationTotalTime()
	{
		animationTotalTime = 0;
	...
	/// <summary>
	/// 現在の時間のタイルアニメーションのタイル番号を取得
	/// </summary>
	/// <returns>タイル番号  または 0=アニメーションなし</returns>
	int GetAnimationTile()
	{
		if (!IsAnimation())
			return 0;
		int ms=TMX::timer.ms();
		int nowTime = ms % GetAnimationTotalTime();	// アニメーションの周期ごと計算にする
		int checkTime = 0;
		for (auto& frame : animation)
		{
			checkTime += frame.duration;
			if (nowTime < checkTime)
				return frame.tileid;
		}
		return animation[animation.size() - 1].tileid;
	}
...
public:
	...
	String source // 別TSXファイルで読み込む場合ファイル名がある。 それ以外使わない
		, name
		, ApptoFolder ;
	// TSXファイルがある場合AppフォルダからのTSXファイルフォルダを覚えておく必要がある
	// そこからの相対パスの画像ファイルなどを読み込む場合があり、パスが変化する
	// 相対パスからの相対パスで、TSXファイルが変われば相対パスも変化するので、
	// この辺の管理も必要
...
*/
	...
	String SourceFolder()
	{
		return source.substr(0,source.lastIndexOf(U'/') + 1);
	}
	...
	{
		if (!IsSource()) return;
		// TSXファイル内で画像のロードがある場合、そこからのファイルパス計算になるので
		// TSXのフォルダを記憶

		/*
		相対パス基本のややこしいのでまとめ
		TMXファイルロード ＞からの～＞  TMXファイルのフォルダから相対パスの TSXファイルロード
		＞＞ それからの～ ザキヤマ
		＞＞ そのTSXファイルのフォルダからの相対パスが画像ファイル<image source>
		
		*/
		TMXfile::SetTSXFolder(source);
		// TSXファイルのロード
		String TSXfile = TMXfile::TMXFolder + Tileset::source;
		XMLReader tsx((FilePathView)(TSXfile));

		TMXfile::SetTSXFolder(TSXfile); // TSXファイルの場所
	...
	void TSXFile_Set_TilesetAttr_and_Tile(XMLReader& tsx_xml)
	{
		...
		while (element)
		{
			...
			if (tag == U"image") // <image >
			{
				...
				TMXfile::SetTSXFolder(image.source); // .pngフォルダ階層があればフォルダ名のみ取得
			}
			if (tag == U"grid")	// <grid>
			{
				//<grid orientation = "orthogonal" width = "1" height = "1" / >
...
public:
	...
	int MapHeight(int layerID)
	{
		return layer[layerID].height;
	}
	int TileWidth(int layerID)
	{
		return tileset[layerID].tilewidth;
	}
	int TileHeight(int layerID)
	{
	...
	void LoadTMXfile(String tmxFile)
	{
		...
		TMXfile::RootFolder = TMXfile::TMXFolder; //test
		...
		for (auto& ts : tileset)	// <tileset> に <image>が無い場合がある
		{
			bool debug = ts.image.IsUse();
			if (ts.image.IsUse())
			{
				// 画像の場所 <tileset source>+<image source >
				//String source00 = TMXfile::GetPNGFile(TMXfile::TSXFolder+ts.source, ts.image.source);
				String imgSource = TMXfile::TMXFolder+ts.SourceFolder()+ts.image.source;
				_ASSERT_EXPR( FileSystem::IsFile(FilePath(imgSource)), L"タイルの画像が読み込めません。ファイル名ミスの可能性があります。");
				TextureAsset::Register(imgSource, imgSource);
				ts.image.tileTexture = TextureAsset(imgSource);
			}
			// tilesetに<image source>がある場合と、
			// 無くて、<tile><image source></tile> にある場合がある
		}

		scrollPosition = { 0,0 };
#ifdef _DEBUG
		auto test0 = Window::ClientSize();
	...
	void TileDraw(int tileNumber, int x, int y)
	{
		...
		auto& tilesetLayer = tileset[tileLayer];
		// アニメーションタイルのチェック 開発中
		// 
		if(tilesetLayer.tile[tileNumber].IsAnimation())
		{
			tileNumber = tilesetLayer.tile[tileNumber].GetAnimationTile();
		}

		tilesetLayer.image.tileTexture
		(GetTileRect(tilesetLayer.firstgid, tileNumber, tileLayer)).draw(x, y);	// ※ 下のは配列外エラーになる

#ifdef _DEBUG
		auto width_dbug=tilesetLayer.image.tileTexture.width();
#endif // _DEBUG
	...
	// columns==0 の時は,１タイル設定時、未実装　
	Rect GetTileRect(int firstGid, int tileNumber, int layerID = 0)
	{
		int tnum = tileNumber - firstGid;
		auto& tl = tileset[layerID];
		int columns = tl.columns, tileWidth = tl.tilewidth, tileHeight = tl.tileheight;
		if (columns == 0) columns=1;
		return Rect{ tnum % columns * tileWidth, tnum / columns * tileHeight, tileWidth, tileHeight };
		//return columns ?
		//	Rect{ tnum % columns * tileWidth, tnum / columns * tileHeight, tileWidth, tileHeight }
	...
	Point RollArrayPosition(Point pos,int layerID=0)
	{
		return {
			UMod(pos.x, MapWidth(layerID) * TileWidth(layerID)) / TileWidth(layerID),
			UMod(pos.y, MapHeight(layerID) * TileHeight(layerID)) / TileHeight(layerID)
		};
	}
	// 円筒スクロール座標変換 引数：ワールド座標　戻り値：マップの配列座標
	...
	Rect GetRect(Point arrayPos,int layerId)
	{
		return Rect(ToPixelPos(ToMapArrayPos(arrayPos)), TileWidth(layerId), TileHeight(layerId));
	}
	// マップ配列座標のピクセルでの表示座標での矩形情報の計算　引数：マップ配列座標　戻り値：ピクセル単位の矩形情報
	Rect GetLocalRect(Point arrayPos, int layerID)
	{
		Point local(ToPixelPos(ToMapArrayPos(arrayPos) - scrollPosition));
		return Rect(local, TileWidth(layerID), TileHeight(layerID));
	}

	// スクロールの左上の微妙なずれ分の計算
	Point Shift(Vec2 scroll, int layerID)
	{
		_ASSERT_EXPR(layerID<tileset.size(),L"tileset[layerID] 範囲外エラー");
#ifdef _DEBUG
		int w=tileset[layerID].tilewidth,h= tileset[layerID].tileheight;
#endif // _DEBUG
		int tileWidth = TileWidth(layerID), tileHeight = TileHeight(layerID);
		Point _shift(-UMod((int)scroll.x, tileWidth), -UMod((int)scroll.y, tileHeight));
		return _shift;
	}

	// マップ配列座標からピクセル座標にする　引数：マップ配列座標　レイヤー層　戻り値：ピクセル座標
	Point ToPixelPos(Point mapPos,int layerID=0)
	{
		return Point(mapPos.x * TileWidth(layerID), mapPos.y * TileHeight(layerID));
	}
	Point ToMapArrayPos(Point pixelPos,int layerID=0)
	{
#ifdef _DEBUG
		auto widthSize=TileWidth(layerID)*MapWidth(layerID);
		auto heightSize=TileHeight(layerID)*MapHeight(layerID);
		auto t0 = TileWidth(layerID), t1 = TileHeight(layerID);
#endif // _DEBUG

		Point arrayPos(pixelPos.x / TileWidth(layerID), pixelPos.y / TileHeight(layerID));
	...
	Rect ToTileRect(Point pixelPos,int layerID=0)
	{
		return Rect(ToPixelPos((ToMapArrayPos(pixelPos))), TileWidth(layerID), TileHeight(layerID));
	}
	...
	void MapRollDraw(int layerIndex,Vec2 scrollSpeed = Vec2(1,1))
	{
		...
		Point drawHomePos = Shift(scroll,layerIndex); // 表示がおかしかったら、バグバグ
		Point drawPos = drawHomePos;

		for (int y = leftTop.y; y < leftTop.y + screenArraySize.y + 2; y++)
		{
			drawPos.x = drawHomePos.x;
			for (int x = leftTop.x; x < leftTop.x + screenArraySize.x + 1; x++, drawPos.x += TileWidth(layerIndex))
			{
				int x0 = x % MapWidth(layerIndex);
				int y0 = y % MapHeight(layerIndex);
				if (IsOutOfMap({ x0,y0 }))continue;
				TileDraw(layer[layerIndex].tileMap[y0][x0], drawPos.x, drawPos.y);
			}
			drawPos.y += TileHeight(layerIndex);
		}
	}
	void MapDrawAll()
	{
	...
	void MapDraw(int layerIndex = 0, Vec2 scrollSpeed = Vec2(1,1))
	{
...
#ifdef _DEBUG
		int w=layer[layerIndex].width,h=layer[layerIndex].height;
#endif // _DEBUG
		if (scroll.x < 0)leftTop.x -= MapWidth(layerIndex);
		if (scroll.y < 0)leftTop.y -= MapHeight(layerIndex);

		Point drawHomePos = Shift(scroll,layerIndex);
		Point drawPos = drawHomePos;
		...
		// バグバグ　screenArraySizeが {0,0} の時がある

		for (int y = top; y < top + screenArraySize.y + 2; y++)
		{
			drawPos.x = drawHomePos.x;
			//			for (int x = leftTop.x; x < leftTop.x + screenArraySize.x + 2; x++, drawPos.x += TileWidth(layerID))
			for (int x = left; x < left + screenArraySize.x + 2; x++, drawPos.x += TileWidth(layerIndex))
			{
