...
// 2023  1 17 OpenSiv3D0.6.6用及びFPS固定ヘッダー導入時の整合性問題の解決
...
// 2021 10/29 00:01 マップのアニメーションタイルの設定が１ずれているのを修正@bug
...
}//https://siv3d.github.io/ja-jp/reference/messagebox/
...
public:
	...
	// 例： U"abcd/efg.tmx" >> TMXFile::UseFolder=U"abcd/"
	static String SetPNGFolder(String fileName, int parent = 0)
	{
	...
	void ReadValue(XMLElement& element)
	{
		...
		for (String tag = child.name(); tag != U""; child = child.nextSibling(), tag = child.name())
		{
			...
			if (tag == U"objectgroup")
			{
				...
				objectGroup.ObjectArraySet(child);
			}
			if (tag == U"properties")
			{
...
*/
	...
	void TSXfileSet()
	{
		...
		TMXfile::RootFolder = TMXfile::TSXFolder;
		// TSXファイルのロード
		XMLReader tsx((FilePathView)(TMXfile::RootFolder+ Tileset::source));
	...
	//			XMLElement* xml=&element;
			...
			*/
			...
			if (tag == U"tile") //<tile～>内すべて
			{
				...
				id = 1 + Parse<int>(element.attribute(U"id").value());
				// bug修正 マップのTMXファイルの値が１多いので １加算している@bug
...
public:
	...
	void LoadTMXfile(String tmxFile)
	{
		...
		TMXfile::RootFolder = TMXfile::TMXFolder;
		...
		for (auto& a : tileset)	// <tileset> に <image>が無い場合がある
		{
			bool debug = a.image.IsUse();
			if (a.image.IsUse())
			{
				// 画像の場所 <tileset source>+<image source >
				String source = TMXfile::GetPNGFile(TMXfile::TMXFolder+a.source, a.image.source);
		...
		scrollPosition = { 0,0 };
		screenArraySize = ToMapArrayPos(Scene::Size());
	...
	void TileDraw(int tileNumber, int x, int y)
	{
		...
		// アニメーションタイルのチェック 開発中
		// 
		if(tilesetLayer.tile[tileNumber].IsAnimation())	//debug
		{
	...
	//	int widthCount = tileset[layerId].image.width / tileset[layerId].tilewidth;
	//	int heightCount = tileset[layerId].image.height / tileset[layerId].tileheight;

	//	return { tn % widthCount * tileset[layerId].tilewidth, tn / widthCount * tileset[layerId].tileheight,
	//		tileset[layerId].tilewidth, tileset[layerId].tileheight };
	...
	Vec2 LocalToWorldPosition(Vec2 pixelPos)
	{
	...
	// ローカル座標（標示座標）の取得 引数：ピクセル座標　　戻り値：ワールド座標
	Vec2 WorldToLocalPosition(Vec2 pixelPos)
	{
		return pixelPos - scrollPosition;
	}
	...
	{
		return {
			UMod(pos.x, MapWidth() * map.tilewidth) / map.tilewidth,
			UMod(pos.y, MapHeight() * map.tileheight) / map.tileheight
		};
	}
	// 円筒スクロール座標変換 引数：ワールド座標　戻り値：マップの配列座標
	...
	*/
	...
	{
		Point _shift(-UMod((int)scroll.x, map.tilewidth), -UMod((int)scroll.y, map.tileheight));
		return _shift;
	}
	...
	Point ToMapArrayPos(Point pixelPos)
	{
		...
		Point arrayPos(pixelPos.x / map.tilewidth, pixelPos.y / map.tileheight);
	...
	void MapDraw(int layerIndex = 0, Vec2 scrollSpeed = Vec2(1,1))
	{
		...
		for (int y = top; y < top + screenArraySize.y + 2; y++)
		{
			...
			for (int x = left; x < left + screenArraySize.x + 2; x++, drawPos.x += map.tilewidth)
			{
	...
	{
		return Rect(scrollPosition.x, scrollPosition.y, Scene::Size()).intersects(pos);	//Window::ClientSize()
	}
